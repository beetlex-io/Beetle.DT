...
namespace Beetle.DTCore.Center
{
	public class ServerCenter : IServerHandler
	{
		...
		public void Close()
		{
			if (mCenterServer != null)
				mCenterServer.Dispose();
			if (mCliServer != null)
				mCliServer.Dispose();
		}
		...
		public void OnRegiste(Register register, PacketDecodeCompletedEventArgs e)
		{
			bool verify = false;

			if (e.Server.Name == "Manager-SERVER")
			{
				verify = register.VerifyToken(this.ManagerKey);
			}
			else
			{
				verify = register.VerifyToken(this.ServerKey);
			}

			if (verify)
			{
				Loger.Process(LogType.INFO, "{0} registed from {1} with {2}", register.Name, e.Session.RemoteEndPoint, e.Server.Name);
				e.Session.Authentication = AuthenticationType.security;
			}
			else
			{
				Loger.Process(LogType.ERROR, "{0} register error from {1} with {2}", register.Name, e.Session.RemoteEndPoint, e.Server.Name);
				e.Session.Dispose();
			}
